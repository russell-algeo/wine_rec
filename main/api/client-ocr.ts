import { createAnalysisFromClientOcrRequestSchema } from "@wine-rec/contracts";
...
export default async function handler(req: VercelRequestLike, res: VercelResponseLike) {
  ...
  try {
    const parsed = createAnalysisFromClientOcrRequestSchema.parse(
      await readJsonBody<unknown>(req),
    );
    const response = await createClientOcrAnalysis({
