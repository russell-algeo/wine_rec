...
import { cancelAnalysis, createUrlAnalysis, getAnalysisRun } from "../../main/src/api-handlers.ts";

const sourceUrl = process.env.BENCHMARK_URL?.trim();
const label = process.env.BENCHMARK_LABEL?.trim() || "benchmark";
const chunkWorkerUrl = process.env.CHUNK_WORKER_BASE_URL?.trim() || process.env.WORKER_URL?.trim();
const coordinatorUrl =
  process.env.BENCHMARK_BASE_URL?.trim() || process.env.VERCEL_URL?.trim() || process.env.WORKER_URL?.trim();
...
if (!coordinatorUrl) {
  throw new Error("BENCHMARK_BASE_URL, VERCEL_URL, or WORKER_URL is required");
}

if (!chunkWorkerUrl) {
  throw new Error("CHUNK_WORKER_BASE_URL or WORKER_URL is required");
}
...
process.env.VERCEL_URL = coordinatorUrl;
process.env.CHUNK_WORKER_BASE_URL = chunkWorkerUrl;
